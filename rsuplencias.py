...
def suplencias():
    ...
    def load_data():
        ...
        path = Path.home() / "ContPerFactura" / "suplencias.xlsx"
    ...
    window = tk.Tk()
    window.title("Registro de Suplencias")
    ...
    for i, col in enumerate(cols):
        ...
        if i == len(cols) - 1:  # Last column for date range
            ...
            entry = ttk.Combobox(input_frame, values=["DESAYUNO", "COMIDA", "CENA"], width=10, state="readonly", name="alimento")
        elif i == len(cols) - 3:  # Antepenultimate column for combobox MR, MIP
            entry = ttk.Combobox(input_frame, values=["MR", "MIP", "GAB", "GBB", "JADB", "JANB", "GAC", "GBC", "JADC","JANC", "GA-IMSSB","GB-IMSSB","JAD-IMSSB","JAN-IMSSB", "GA-HRF","GB-HRF","JAD-HRF","JAN-HRF","JAD-PASANTES","CRUM"], width=15, state="readonly", name="tipo")
        else:
