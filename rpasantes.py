...
def pasantes():
    ...
    def load_data():
        ...
        path = Path.home() / "ContPerFactura" / "pasantes.xlsx"
    ...
    window = tk.Tk()
    window.title("REGISTRO DE PASANTES")
    ...
    for i, col in enumerate(cols):
        ...
        if i == len(cols) - 1:  # Last column for date range
            ...
            entry = ttk.Combobox(input_frame, values=["DESAYUNO", "COMIDA"], width=10, state="readonly", name="alimento")
        elif i == len(cols) - 3:  # Antepenultimate column for combobox 
            entry = ttk.Combobox(input_frame, values=["JAD-PASANTES"], width=15, state="readonly", name="tipo")
            entry.set("JAD-PASANTES")  # Establece el valor predeterminado
        else:
