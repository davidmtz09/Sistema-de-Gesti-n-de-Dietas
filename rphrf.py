...
def phrf():
    ...
    def load_data():
        ...
        path = Path.home() / "ContPerFactura" / "phrf.xlsx"
    ...
    window = tk.Tk()
    window.title("HOMOLOGADOS, REGULARIZADOS Y FORMALIZADOS")
    ...
    for i, col in enumerate(cols):
        ...
        if i == len(cols) - 1:  # Last column for date range
            ...
            entry = ttk.Combobox(input_frame, values=["GA-HRF","GB-HRF","JAD-HRF","JAN-HRF"], width=10, state="readonly", name="tipo")
        else:
