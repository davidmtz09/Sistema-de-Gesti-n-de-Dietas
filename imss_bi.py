...
def imss():
    ...
    def load_data():
        ...
        path = Path.home() / "ContPerFactura" / "imssb.xlsx"
    ...
    window = tk.Tk()
    window.title("Registro de Personal IMSS Bienestar")
    ...
    for i, col in enumerate(cols):
        ...
        if i == len(cols) - 1:  # Last column for date range
            ...
            entry = ttk.Entry(input_frame, width=20, name="tipo")
            entry = ttk.Combobox(input_frame, values=["GA-IMSSB","GB-IMSSB","JAD-IMSSB","JAN-IMSSB"], width=10, state="readonly", name="tipo")
        else:
